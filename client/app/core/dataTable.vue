	...
	<table class="table table-striped">
		<thead>
			<tr>
				...
				<th class="sortable" v-for="col in schema.columns" v-bind:key="col" :width="col.width || 'auto'" @click="changeSort(col)" :class="{ sorted: col.field == order.field, 'desc': col.field == order.field &amp;&amp; order.direction == -1 }">{{ col.title }}</th>
			</tr>
		</thead>
		<tbody>
			<tr v-for="row in filteredOrderedRows" v-bind:key="row" @click="select($event, row)" :class="getRowClasses(row)">
				<td class="selector" v-if="schema.multiSelect" width="20px" @click.stop.prevent="select($event, row, true)"><i class="far fa-square"></i></td>
				<td v-for="col in schema.columns" v-bind:key="col" :class="getCellClasses(row, col)">
					<span v-html="getCellValue(row, col)"></span>
					<span class="labels" v-if="col.labels != null">
						<div class="label" v-for="label in col.labels(row)" v-bind:key="label" :class="'label-' + label.type">{{ label.caption }}
						</div>
					</span>
				</td>
			</tr>
		...
		<tfoot>
			<tr>
				<td></td>
				<td v-for="col in schema.columns" v-bind:key="col">&nbsp;</td>
			</tr>
		</tfoot>
	</table>
</template>
